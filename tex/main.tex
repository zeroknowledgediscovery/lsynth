...
While MAP-alignment fidelity $\Upsilon(D)$ quantifies preservation of the
\emph{conditional structure} of the data distribution, it does not by itself
address row-level memorization. In particular, a synthetic dataset may achieve
high $\Upsilon$ while containing records that are nearly identical to specific
real samples. To explicitly characterize this failure mode, we introduce a
simple row-level novelty diagnostic.
...
\end{equation}
We summarize row-level memorization via the \emph{lack-of-novelty score}
\begin{equation}
...
record. Tail statistics of $\{\eta_j\}$ (e.g., the $95^{\mathrm{th}}$ percentile)
capture worst-case near-copying behavior.

This diagnostic is orthogonal to MAP-alignment fidelity:
$\Upsilon(D)$ measures whether synthetic data respect learned conditional
dependencies, whereas $\mathcal{N}(D_{\mathrm{syn}})$ measures proximity to
specific real records. High-quality synthetic data should therefore exhibit
high $\Upsilon$ and low $\mathcal{N}$. 


So the ideal synthtic dataset should achieve  $\Upsilon \rightarrow 1$, with $\mathcal{N} $ substantially lower than $1$. 
